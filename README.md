...
### Interface binding
On hosts connected to several networks, the routing table alone cannot always reach every gateway — most sharply when two networks use identical private address ranges. Both `GatewayAddress` variants therefore accept an optional network interface index (for IPv6 this is the scope id, which link-local addressing already requires): sockets toward a gateway with a known interface are bound to it (`SO_BINDTOIFINDEX`/`IP_BOUND_IF`), the announcement listener joins the multicast group on it, and UPnP control connections and event callbacks are confined to it. Indices name one interface instance and are not promptly reused, so a bound socket whose interface disappears fails with an error instead of silently moving to another network; re-attempt with a fresh index. Interface binding exists on Linux, Android, Apple, and Solaris-family platforms — Linux kernels older than 5.7 additionally require the `CAP_NET_RAW` capability, and socket creation fails without it rather than sending traffic over an unintended network. On other platforms sockets are not bound, but the index still selects the interface the announcement listener joins multicast groups on where joining by index is supported, and the IPv6 link-local scope everywhere.
